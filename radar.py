...
class Radar:
    ...
    def capture_position_variations(self, position, time_reading):
        ...
        print(
            f"[RADAR] Received drone position and time: {position} at time {time_reading:.2f}s"
        )
    ...
    def start_sending_updates_to_interceptor(self, interceptor, update_interval):
        ...
        if not self.is_sending_updates_to_interceptor:
            ...
            self.interceptor_update_thread = Thread(
                target=self._send_updates_to_interceptor, args=(update_interval,)
            )
    ...
    def _send_updates_to_interceptor(self, update_interval):
        while self.is_sending_updates_to_interceptor:
            if self.interceptor:
                ...
                print(
                    f"[RADAR] Sending position to interceptor: {latest_position} at time {latest_time:.2f}s"
                )
                self.interceptor.update_position_from_radar(
                    latest_position, latest_time
                )
