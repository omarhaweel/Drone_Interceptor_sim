...
def plot_3d_trajectory(drone, interceptor):
    ...
    ax.plot(
        positions_interceptor[:, 0],
        positions_interceptor[:, 1],
        positions_interceptor[:, 2],
        "r-",
        linewidth=2,
    ...
    ax.scatter(
        positions_interceptor[0, 0],
        positions_interceptor[0, 1],
        positions_interceptor[0, 2],
        color="orange",
        s=100,
    ...
    ax.scatter(
        positions_interceptor[-1, 0],
        positions_interceptor[-1, 1],
        positions_interceptor[-1, 2],
        color="purple",
        s=100,
    ...
    ax.quiver(
        current_pos_interceptor[0],
        current_pos_interceptor[1],
        current_pos_interceptor[2],
        interceptor.velocity[0],
        interceptor.velocity[1],
        interceptor.velocity[2],
        color="blue",
        arrow_length_ratio=0.1,
        label="Interceptor Velocity",
    )
...
def animate(drone, interceptor, interval=100):
    ...
    (line_interceptor,) = ax.plot(
        [], [], [], "r-", linewidth=1, label="Interceptor Trajectory"
    )

    # Initialize points for current positions
    (point_drone,) = ax.plot([], [], [], "bo", markersize=10, label="Drone Position")
    (point_interceptor,) = ax.plot(
        [], [], [], "ro", markersize=6, label="Interceptor Position"
    )
    ...
    def animate_frame(frame):
        ...
        if frame < len(positions_drone):
            line_drone.set_data(
                positions_drone[: frame + 1, 0], positions_drone[: frame + 1, 1]
            )
            line_drone.set_3d_properties(positions_drone[: frame + 1, 2])
            point_drone.set_data(
                [positions_drone[frame, 0]], [positions_drone[frame, 1]]
            )
            point_drone.set_3d_properties([positions_drone[frame, 2]])
            current_time = (
                times_drone[frame] if frame < len(times_drone) else times_drone[-1]
            )
        else:
        ...
        if frame < len(positions_interceptor):
            line_interceptor.set_data(
                positions_interceptor[: frame + 1, 0],
                positions_interceptor[: frame + 1, 1],
            )
            line_interceptor.set_3d_properties(positions_interceptor[: frame + 1, 2])
            point_interceptor.set_data(
                [positions_interceptor[frame, 0]], [positions_interceptor[frame, 1]]
            )
            point_interceptor.set_3d_properties([positions_interceptor[frame, 2]])
        else:
            # Keep last position visible
            point_interceptor.set_data(
                [positions_interceptor[-1, 0]], [positions_interceptor[-1, 1]]
            )
            point_interceptor.set_3d_properties([positions_interceptor[-1, 2]])

        ax.set_title(
            f"Animated Drone and Interceptor Movement - Time: {current_time:.2f}s"
        )
